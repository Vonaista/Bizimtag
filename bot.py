...
async def help(event):
  helptext = "**Rapid T@g Bot'un Yardım Menüsü**\n\nKomut: /all \n  Bu komutu, başkalarına bahsetmek istediğiniz metinle birlikte kullanabilirsiniz. \n`Örnek: /all Günaydın!`  \nBu komutu yanıt olarak kullanabilirsiniz. herhangi bir mesaj için Bot, yanıtlanan iletiye kullanıcıları etiketleyecek ve /bitir yazarak etiketleme işlemi bitecek. /ytag Komutuyla Sadece Yöneticileri Etiketleyebilirsiniz 🤗"
...
async def help(event):
  helptext = "**Çok özellikleri Etiket Botu Bulmaya Çalışan Grub Sahibleri Rapid T@g Size Göre:\n\n📌 5-li etiket\n📌 Emoji etiket\n📌 Tekli Etiket\n📌 Yalnız Yöneticileri etiketleme\n📌\n\n Böyle Çok özellikli TaggerBot 'u grubunuza yönetici olarak ekleyip rahatlıkla üyelir , etiket ata bilirsiz **"
  await event.reply(helptext,
                    buttons=(
                      [Button.url('Botu Gruba Ekle➕', 'https://t.me/RapidTagBot?startgroup=a')],
                    ),
                    link_preview=False
                   )
...
@client.on(events.NewMessage(pattern="^/ytag ?(.*)"))
async def mentionall(event):
...
print(">> Bot çalıyor merak etme 🚀 @EfsaneLions bilgi alabilirsin <<")
